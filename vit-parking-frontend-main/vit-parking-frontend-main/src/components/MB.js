...
const Temp = (userID) => {
  ...
  // console.log("Running.... ");
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(backend+"/mb");
        if (!response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    if (!isLoggedIn) navigate("/login", { state: { returnPath: "/mainbuilding" } });
    else {
      const userString = localStorage.getItem("user");
      const user = JSON.parse(userString);
      const username = user.username;
    ...
    <>
      <div className="container-fluid">
        <div className="row sub-headings">
          ...
          <div className="offset-2 col-6"><p>Main Building</p></div>
            ...
            <div className="col-12 text-white fs-3 fw-bold ">
              ...
              <div style={{ width: "100%" }}>
                <iframe
                  ...
                  marginheight="0"
                  marginwidth="0"
                  src="https://maps.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q=12.968636682256378,%2079.15504579829071+(VIT%20VELLORE%20SJT)&amp;t=&amp;z=18&amp;ie=UTF8&amp;iwloc=B&amp;output=embed"
                >
