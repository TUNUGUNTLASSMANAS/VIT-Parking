...
const Temp = (userID) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
      // console.log("Running.... SJT");
        const token = localStorage.getItem("token");
        if (token) setIsLoggedIn(true);
        const response = await fetch(backend+"/sjt");
        if (!response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    if (!isLoggedIn) navigate("/login", { state: { returnPath: "/sjt" } });
    else {
      const userString = localStorage.getItem("user");
      const user = JSON.parse(userString);
      const username = user.username;
    ...
    <>
      <div className="container-fluid">
        <div className="row sub-headings">
          ...
          <div className="offset-2 col-6"><p>Silver Jubilee Tower</p></div>
          <div className="offset-2 col-1"><button className={isLoggedIn?"btn btn-danger":"btn btn-primary"} 
            onClick={()=>{
              if (isLoggedIn) {
            ...
            <div className="col-12 text-white fs-3 fw-bold ">
              ...
              <div style={{ width: "100%" }}>
                <iframe
                  ...
                  marginheight="0"
                  marginwidth="0"
                  src="https://maps.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q=12.971461208934462,%2079.16441599965314+(VIT%20VELLORE%20SJT)&amp;t=&amp;z=18&amp;ie=UTF8&amp;iwloc=B&amp;output=embed"
                >
